# Product Positioning

> Навигация: [README.md](../../README.md) · Бизнес-модель: [Business mode/marketing/gtm-manifesto.md](../../../Business%20mode/marketing/gtm-manifesto.md)

---

## Summary

**One-liner:** система анализирует коммуникации ОП, находит просадки до P&L, развивает команду в процессе работы.

**Отличие от CRM / ОКК:** pipeline + анализ качества + база знаний + развитие людей — в одном контуре.

**ICP:** ОП с большим объёмом коммуникаций — от малых групп до департаментов 100–300+.

---

## Детали

| Тема | Документ |
|------|----------|
| ICP-сегменты (Primary / Secondary / Anti) | [docs/positioning/icp-segments.md](../../docs/positioning/icp-segments.md) |
| Value proposition, pillars, positioning | [docs/positioning/value-proposition.md](../../docs/positioning/value-proposition.md) |
| Конкурентная карта | [docs/positioning/competitive-map.md](../../docs/positioning/competitive-map.md) |
| Продуктовые принципы, открытые вопросы | [docs/positioning/product-principles.md](../../docs/positioning/product-principles.md) |
